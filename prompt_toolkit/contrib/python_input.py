...
class PythonLine(Line):
    ...
    """
    multiline_string_delims = re.compile('''[']{3}|["]{3}''')
    ...
    @property
    def ends_in_multiline_string(self):
        """
        Determine if we're inside a multiline string at hte end of the line.
        """
        delims = PythonLine.multiline_string_delims.findall(self.text)
        opening = None
        for delim in delims:
            if opening is None:
                opening = delim
